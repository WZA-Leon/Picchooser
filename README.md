...
Picchooser/
├── Picchooser.py        # 主程序入口
├── tutorial.txt         # 「软件教程」文本（主菜单 [5] 显示的内容）
...
- **`5` 软件教程**：显示使用说明与当前配置（内容来自 `tutorial.txt`，可自行编辑），回车返回主菜单。
- **`0` 退出**：结束程序。
...
| 快捷方式（.lnk） | 青色 | 指向文件夹的 Windows 快捷方式，显示为 `快捷方式名 → 目标路径`，选中后进入其目标目录 |
